...
import { deleteBrand, updateBrandStatus } from "./brandSlice";
...
const ToolBarBrand = ({
  ...
  const statusOptions = useMemo(() => {
    ...
    const allStatuses = brands.map((u) => u.status).filter(Boolean);
        ...
        <BulkActionBar
          ...
          onApply={async ({ action, selectedIds }) => {
            try {
              if (action === "delete") {
                await dispatch(
                  deleteBrand({ brandIds: selectedIds })
                ).unwrap();
                toast.success("Đã xóa danh mục thành công!");
              } else {
                for (const brandId of selectedIds) {
                  await dispatch(
                    updateBrandStatus({ brandId, status: action })
                  ).unwrap();
                }
                toast.success(
                  action === "ACTIVE"
